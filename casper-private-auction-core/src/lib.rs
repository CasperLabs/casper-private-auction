...
pub fn auction_bid() {
    ...
    let bid = runtime::get_named_arg::<U512>(data::BID);
    if bid < AuctionData::get_reserve() {
        runtime::revert(AuctionError::BidTooLow);
    }
    let bidder_purse = runtime::get_named_arg::<URef>(data::BID_PURSE);
    ...
    if !AuctionData::is_english_format() {
        if let (None, None) = (winner, price) {
            ...
            add_bid(bidder, bidder_purse, AuctionData::get_current_price());
...
pub fn auction_cancel_bid() {
    ...
    if block_time < cancellation_time {
        ...
        match bids.get(&bidder) {
            Some(current_bid) => {
                let auction_purse = AuctionData::get_auction_purse();
                system::transfer_from_purse_to_account(auction_purse, bidder, *current_bid, None)
                    .unwrap_or_revert();
                bids.remove(&bidder);
                AuctionData::update_bids(bids);
...
fn auction_allocate(winner: Option<AccountHash>) {
    match winner {
        ...
        _ => auction_transfer_token(AuctionData::get_token_owner()),
    }
}
